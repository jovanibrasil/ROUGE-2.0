...
public class SettingsUtil {
	...
	public static void loadProps(ROUGESettings rs) {
		...
		try {
			...
			System.err.println(
					"Properties file not found. Please specify -Drouge.prop=<path_to_rouge_prop> in the command line or place 'rouge.properties' file in the root of the project.");
			System.exit(-1);
		}
	...
	public static void loadProps(InputStream stream, ROUGESettings rs) {
		...
		try {
			...
			val = properties.getProperty("single_evaluation", "false");
			rs.SINGLE_EVALUATION = Boolean.parseBoolean(val);
			
			val = properties.getProperty("reference_file.dir", "");
			rs.REFERENCE_FILE_DIR = val;
			
			val = properties.getProperty("generated_file.dir", "");
			rs.GENERATED_FILE_DIR = val;
